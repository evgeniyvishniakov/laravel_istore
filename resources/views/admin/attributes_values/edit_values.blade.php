            ...
            <div class="col-md-6 offset-md-2 col-sm-2">
                <div class="card">
                    <div class="card-header">
                        <h2>Редагування значення</h2>
                    </div>
                    <div class="card-body card-block">
                        <form action="{{ route('attribute.value.update', ['attribute_id' => $attribute->id, 'value_id' => $value->id]) }}" method="post" enctype="multipart/form-data" class="form-horizontal">
                            @csrf
                            @method('PUT')
                            <div class="row form-group">
                                <div class="col col-md-3"><label for="name" class=" form-control-label">Назва</label></div>
                                <div class="col-12 col-md-9"><input  type="text" id="name" name="name" value="{{ $value['name'] }}" required class="form-control"></div>
                            </div>
                            @if($errors->has('name'))
                            ...
                            <div class="row form-group">
                                ...
                                <div class="col-12 col-md-9"><input type="text" value="{{ $value['slug'] }}" id="slug" name="slug" class="form-control"></div>
                            </div>
                            @if($errors->has('slug'))
                                <div class="alert alert-danger">
                                    <ul>
                            ...
                            @endif
                            <button class="btn btn-success" type="submit">Редагувати значення</button>
                        </form>
                    </div>
                </div>
            </div>
